...
| Negative-gm cross-coupled pair (device class: CMOS pair vs. SiGe HBT pair vs. Colpitts) | **Decided**: `spec/decision-records/DR-002-negative-gm-pair-device-class.md` — a SiGe HBT cross-coupled differential pair in the high-BVCEO `npn13G2v` flavor on the 3.3 V rail, over a CMOS pair or Colpitts | No sibling has a negative-gm topology of any kind; the ring VCOs' constant-gm bias core (§3 below) is a different circuit function entirely. The tank study's Q/SRF evidence this row gated on has landed: `sim/tank-characterization/` finding 7 (`Q_L ≈ 5` upper bound at 10 GHz — the inductor, not the cap, sets tank Q) and `sim/inductor-model/em-extraction/` (multi-turn SRF 8.07/9.42 GHz, 50–77 % below analytic) supplied the low-Q-tank gm-margin evidence DR-002 weighs | `spec/target-spec.md` rows 0 (pair supply flavor, with rows 6 startup margin) now cite DR-002 as their condition; absolute gm-vs-tank-loss margins still await the row-6 startup testbench against the PDK's VBIC HBT corner models, per DR-002's own stated scope; the HBT choice also carries a named permanent LVS-recognition consequence (klayout-tools#1242, per DR-002 "Consequences") |
| Varactor / tuning mechanism (MOS accumulation-mode vs. junction varactor; band-switching capacitor bank or none) | **Decided**: `spec/decision-records/DR-001-tuning-mechanism.md` — `sg13_hv_svaricap` (MOS accumulation-mode) over `dantenna`/`dpantenna`, driven over its full 0.0–3.3 V HV `Vctrl` domain; no band-switching capacitor bank adopted in v1 (named as an open trigger, not closed) | Neither ring-VCO sibling has a varactor at all (ring tuning is via current starving, an unrelated mechanism); `sim/varactor-characterization/` (#18/#19) supplied the `C(V)`/`Q(V,f)` evidence DR-001 weighs | `spec/target-spec.md` row 2 (and rows 0, 3, 9) now cite DR-001 as their condition; absolute Kvco range still awaits the tank-characterization study, per DR-001's own stated scope |
