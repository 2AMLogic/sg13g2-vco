...
| 2 | Tuning range (Kvco integral) | | | | Varactor device is decided (`spec/decision-records/DR-001-tuning-mechanism.md`: `sg13_hv_svaricap`, measured `Cmax/Cmin` 1.88–2.08x over the full HV domain, no band-switching bank adopted in v1); the absolute range in Hz still depends on the tank-characterization study's center frequency and usable capacitance, not yet supplied — see `spec/porting-plan.md` §3 | DRAFT — blank pending tank study |
| 3 | Kvco (small-signal slope) and its linearity across the tuning range | | | | Device and `Vctrl` domain decided (`spec/decision-records/DR-001-tuning-mechanism.md`); the `dC/dV` kernel is measured per `sim/varactor-characterization/records/20260909-231619-de50891-kvco.csv`, but the absolute Kvco (Hz/V) still depends on the tank-characterization study's inductor/total-capacitance sizing, not yet supplied. Stated per band if band-switched (DR-001: not adopted in v1); a single Kvco number without a stated Vctrl window is not fillable | DRAFT — blank pending tank study |
...
| 9 | Supply sensitivity (pushing, MHz/V) | | | | Varactor device and `Vctrl` domain decided (`spec/decision-records/DR-001-tuning-mechanism.md`) — the pushing mechanism couples supply noise into `sg13_hv_svaricap`'s well/substrate `Vctrl` node specifically; the number itself still requires tank sizing and a stated coupling path, not yet supplied. An LC tank's pulling/pushing mechanism differs structurally from a current-starved ring's; do not assume the PLL siblings' row/budget shape ports without re-deriving the mechanism | DRAFT — blank pending tank study |
