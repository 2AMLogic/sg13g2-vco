...
| [`oscillator-core/`](oscillator-core/) | the first experiment here that contains an **oscillator**: start-up from the differential initial condition, `f_osc` over the full 0.0–3.3 V `Vctrl` domain (hence the row-2 tuning ratio and the row-3 `Kvco` curve), the large-signal supply current, the differential-mode check, and the row-6 startup margin — all against the committed `design/vco.spice`, over the row-10/11 PVT grid | Bench, method and records shipped; **the graded PVT grid has not run.** Extractor known-answer check: 56/56 quantities within tolerances *derived* from the estimator's own discretization bounds. Single-point calibration probe at the nominal corner: committed. The declared grid (135 PVT × 10 `Vctrl` + 231 margin transients) is several hundred CPU-hours at ~13 ps circuit time per wall-clock second, i.e. a fleet job — and `klt sim` cannot express this deck (no OSDI device loading, no `ngbehavior` selection, no `ihp-sg13g2` off-host image), reproduced in [`oscillator-core/klt-sim/`](oscillator-core/klt-sim/) and filed as [klayout-tools#2511](https://github.com/2AMLogic/klayout-tools/issues/2511). **So rows 1/2/3/6/8 are instrumented but not yet measured**, and no phase-noise (row 4) claim is made. Ratifies no `spec/target-spec.md` row. |
